...
the two Windows matrices and adds `--managed-endpoint` to that build. The
upstream Windows build and packaging steps produce the x64 EXE and MSI and
attach them to a GitHub pre-release. No signing secrets are passed to the
reusable workflow, so the packages are unsigned.

The `managed-endpoint` feature builds an incoming-only Windows endpoint. It
suppresses the viewer and tray, uses a headless connection manager for
unattended access, locks unwanted capabilities, disables unused API and remote
printer startup, and packages without user shortcuts. Consent mode retains the
normal connection window because `--cm-no-ui` cannot display an approval
prompt.
